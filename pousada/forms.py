...
class CasadePedraCotacaoForm(forms.ModelForm):
    ...
    kidyears = forms.ChoiceField(label='Idade da Criança', choices=CHOICES4, widget=forms.Select)
    kidquestion = forms.ChoiceField(label='Tem Criança', choices=CHOICES5, widget=forms.Select)

    class Meta:
        model = CasadePedraCotacao
        fields = ['author', 'name', 'tel', 'checkin', 'checkout', 'person_num', 'kidquestion', 'kidyears', 'discount', 'portion']
